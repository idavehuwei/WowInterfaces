...
function CalendarArenaTeamContextMenu_Initialize(self)
    ...
    for i = 1, MAX_ARENA_TEAMS do
        ...
        if (teamName) then
            UIMenu_AddButton(CalendarArenaTeamContextMenu, -- menu
...
function CalendarDayButton_OnEnter(self)
    ...
    for i = 1, numEvents do
        ...
        if (title and sequenceType ~= "ONGOING") then
            ...
            GameTooltip:AddDoubleLine(format(CALENDAR_CALENDARTYPE_NAMEFORMAT[calendarType][sequenceType], title),
                eventTime,
...
function CalendarEventInviteListButton_OnEnter(self)
    if (self.inviteIndex) then
        ...
        if (weekday ~= 0) then
            ...
            -- date
            GameTooltip:AddLine(format(FULLDATE, _CalendarFrame_GetFullDate(weekday, month, day, year)),
                HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
            -- time
            GameTooltip:AddLine(GameTime_GetFormattedTime(hour, minute, true),
                HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
            GameTooltip:Show();
        end
    end
