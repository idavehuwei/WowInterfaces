...
local GridStatusHots = Grid:GetModule("GridStatus"):NewModule("GridStatusHots")
...
local sashield_hotcolors = {
	...
	["folshow"] = {
		...
		get = function () return GridStatusHots.db.profile.alert_sashield.folshow end,
		set = function (arg)
			GridStatusHots.db.profile.alert_sashield.folshow = arg
		end,
	},
}
...
local function updateUnit(self, gridid, unitid)
	...
	for i=1,999 do
		...
		elseif bname == spellNameCache["Rejuvenation"] then
			total_hots = total_hots + 1;
			if (bismine == "player" and btime and btype == "Magic") then rjtime = btime end
