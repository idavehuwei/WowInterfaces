...
if not GridRoster then
	print("GridStatusThreat requires GridRoster.")
	print("Please update Grid.")
	return
end
local L = AceLibrary("AceLocale-2.2"):new("GridStatusThreat")
local GridStatus = Grid:GetModule("GridStatus");
local GridStatusThreat = GridStatus:NewModule("GridStatusThreat")
...
function GridStatusThreat:PLAYER_REGEN_DISABLED()
	if not self:IsEventScheduled("Threat_Refresh") then
		self:ScheduleRepeatingEvent("Threat_Refresh", self.ThreatUpdate, 0.5, self)
	end
end

function GridStatusThreat:PLAYER_REGEN_ENABLED()
	if self:IsEventScheduled("Threat_Refresh") then
		self.core:SendStatusLostAllUnits("alert_threat_graphic")
		self.core:SendStatusLostAllUnits("alert_threat_text")
		self:CancelScheduledEvent("Threat_Refresh")
	end
end

local function updateUnit(self, guid, unitid)
	local settings = self.db.profile.alert_threat_graphic
	...
	if threatPercent >= opt.threat1 then
		...
		self.core:SendStatusGained(guid, "alert_threat_graphic",
			settings.priority,
			(settings.range and 40),
			statuscolorGraphic,
			nil)
		self.core:SendStatusGained(guid, "alert_threat_text",
			settings.priority,
			(settings.range and 40),
			statuscolorText,
			threatText)
	else
		self.core:SendStatusLost(guid, "alert_threat_graphic")
		self.core:SendStatusLost(guid, "alert_threat_text")
	end
end

function GridStatusThreat:ThreatUpdate()
