...
function GridClickSetsFrame_OnLoad(self)
	...
	for i = 1, 8 do
		dwGetglobal("GridClickSetButton"..i.."Title"):SetText(GridClickSets_Titles[i])
	end
...
function GridClickSetsFrame_OnEvent(self, event)
	...
	elseif(event=="VARIABLES_LOADED") then
		if(GetLocale()=="zhCN") then 
			DEFAULT_CHAT_FRAME:AddMessage("Grid团队框体整合包 由|cff3399FFWarbaby|r@梦境之树|cffFF00FF<冰封十字军>|r倾情奉献.") 
		elseif(GetLocale()=="zhTW") then
			DEFAULT_CHAT_FRAME:AddMessage("Grid團隊框體整合包 由|cff3399FF三月十二|r@聖光之願|cffFF00FF<冰封十字軍>|r傾情奉獻.") 
		end
	end
end

function GridClickSetsFrame_TypeUpdate(id)
	local argF = dwGetglobal("GridClickSetButton"..id.."Arg")
	local typeDD = dwGetglobal("GridClickSetButton"..id.."Type")
	...
	local below = dwGetglobal("GridClickSetButton"..(id+1))
	if(below) then
		below:ClearAllPoints();
		if argF:IsVisible() then
			below:SetPoint("TOPLEFT", argF, "BOTTOMLEFT", -50, 0);
		else
			below:SetPoint("TOPLEFT", dwGetglobal("GridClickSetButton"..id), "BOTTOMLEFT", 0, -5);
		end
	end
end
...
function GridClickSetsFrame_LoadSet(set)
	for i = 1, 8 do
		local dd = dwGetglobal("GridClickSetButton"..i.."Type")
		...
		if(atts.arg) then
			dwGetglobal("GridClickSetButton"..i.."Arg"):SetText(atts.arg)
		else
			dwGetglobal("GridClickSetButton"..i.."Arg"):SetText("")
		end
	end
...
function GridClickSetsFrame_SaveOnClick()
	for i=1,8 do
		GridClickSets_Set[tostring(GridClickSetsFrame.selectedTab)][GridClickSets_Modifiers[i]] = {
			type = UIDropDownMenu_GetSelectedValue( dwGetglobal("GridClickSetButton"..i.."Type") ),
			arg = dwGetglobal("GridClickSetButton"..i.."Arg"):GetText()
		}
...
function GridClickSetsFrame_UpdateGridFrame()
	if(InCombatLockdown()) then
		...
		Grid:GetModule("GridFrame"):WithAllFrames(function (f) GridClickSets_SetAttributes(f.frame, GridClickSets_Set) end)
