...
ATLASLOOT_VERSION = AL["|cffFF8400AtlasLoot Enhanced v"] .. VERSION_MAJOR .. "." .. VERSION_MINOR .. "." .. VERSION_BOSSES .. "|r";
...
function AtlasLoot_OnVariablesLoaded()
    ...
    elseif (Cosmos_RegisterButton) then
        Cosmos_RegisterButton(string.sub(ATLASLOOT_VERSION, 11, 28),
    ...
    panel = dwGetglobal("AtlasLootOptionsFrame");
    ...
    panel = dwGetglobal("AtlasLootHelpFrame");
    panel.name = AL["Help"];
...
function AtlasLoot_ShowItemsFrame(dataID, dataSource, boss, pFrame)
    ...
    for i = 1, 30, 1 do
        dwGetglobal("AtlasLootItem_" .. i .. "_Unsafe"):Hide();
        dwGetglobal("AtlasLootMenuItem_" .. i):Hide();
        dwGetglobal("AtlasLootItem_" .. i):Hide();
        dwGetglobal("AtlasLootItem_" .. i).itemID = 0;
        dwGetglobal("AtlasLootItem_" .. i).spellitemID = 0;
    end
    ...
    if (dataID == "SearchResult") or (dataID == "WishList") or (AtlasLoot_IsLootTableAvailable(dataID)) then
        ...
        for i = 1, 30, 1 do
            ...
            if (dataSource[dataID][i] ~= nil and dataSource[dataID][i][4] ~= "") then
                ...
                itemButton = dwGetglobal("AtlasLootItem_" .. dataSource[dataID][i][1]);
                iconFrame = dwGetglobal("AtlasLootItem_" .. dataSource[dataID][i][1] .. "_Icon");
                nameFrame = dwGetglobal("AtlasLootItem_" .. dataSource[dataID][i][1] .. "_Name");
                extraFrame = dwGetglobal("AtlasLootItem_" .. dataSource[dataID][i][1] .. "_Extra");
                ...
                if dataSource[dataID][i][2] == 0 then dwGetglobal("AtlasLootItem_" .. i .. "_Unsafe"):Hide(); end
            end
        end
        ...
        for i = 1, 30, 1 do
            itemID = dwGetglobal("AtlasLootItem_" .. i).itemID;
            if itemID and itemID ~= 0 and (string.sub(itemID, 1, 1) ~= "s") then
                if GetItemInfo(itemID) then
                    dwGetglobal("AtlasLootItem_" .. i .. "_Unsafe"):Hide();
                else
                    dwGetglobal("AtlasLootItem_" .. i .. "_Unsafe"):Show();
                end
            else
                dwGetglobal("AtlasLootItem_" .. i .. "_Unsafe"):Hide();
            end
        ...
        dwGetglobal("AtlasLootItemsFrame_BACK"):Hide();
        dwGetglobal("AtlasLootItemsFrame_NEXT"):Hide();
        dwGetglobal("AtlasLootItemsFrame_PREV"):Hide();
        ...
        if dataID == "SearchResult" or dataID == "WishList" then
            if wlPage < wlPageMax then
                dwGetglobal("AtlasLootItemsFrame_NEXT"):Show();
                dwGetglobal("AtlasLootItemsFrame_NEXT").lootpage = dataID .. "Page" .. (wlPage + 1);
            end
            if wlPage > 1 then
                dwGetglobal("AtlasLootItemsFrame_PREV"):Show();
                dwGetglobal("AtlasLootItemsFrame_PREV").lootpage = dataID .. "Page" .. (wlPage - 1);
            end
        else
            local tablebase = dataSource[dataID];
            if tablebase.Next then
                dwGetglobal("AtlasLootItemsFrame_NEXT"):Show();
                dwGetglobal("AtlasLootItemsFrame_NEXT").lootpage = tablebase.Next;
            end
            if tablebase.Prev then
                dwGetglobal("AtlasLootItemsFrame_PREV"):Show();
                dwGetglobal("AtlasLootItemsFrame_PREV").lootpage = tablebase.Prev;
            end
            if tablebase.Back then
                dwGetglobal("AtlasLootItemsFrame_BACK"):Show();
                dwGetglobal("AtlasLootItemsFrame_BACK").lootpage = tablebase.Back;
            end
        end
    end
...
function AtlasLoot_GenerateAtlasMenu(dataID, pFrame)
    ...
    for i = 1, 30, 1 do
        ...
        if (AtlasLoot_Data[dataID][i] ~= nil and AtlasLoot_Data[dataID][i][4] ~= "") then
            ...
            --Use shortcuts for easier reference to parts of the item button
            itemButton = dwGetglobal("AtlasLootMenuItem_" .. AtlasLoot_Data[dataID][i][1]);
            iconFrame = dwGetglobal("AtlasLootMenuItem_" .. AtlasLoot_Data[dataID][i][1] .. "_Icon");
            nameFrame = dwGetglobal("AtlasLootMenuItem_" .. AtlasLoot_Data[dataID][i][1] .. "_Name");
            extraFrame = dwGetglobal("AtlasLootMenuItem_" .. AtlasLoot_Data[dataID][i][1] .. "_Extra");
    ...
    if AtlasFrame and AtlasFrame:IsVisible() then
        if ATLAS_CUR_LINES then
            for i = 1, ATLAS_CUR_LINES do
                if dwGetglobal("AtlasBossLine" .. i .. "_Selected"):IsVisible() then
                    dwGetglobal("AtlasBossLine" .. i .. "_Selected"):Hide();
                    dwGetglobal("AtlasBossLine" .. i .. "_Loot"):Show();
                end
            end
        end
    end
...
function AtlasLoot_ShowQuickLooks(button)
    ...
    if dewdrop:IsOpen(button) then
        ...
        local setOptions = function()
            dewdrop:AddLine("text", AL["QuickLook"] .. " 1",
            ...
            dewdrop:AddLine("text", AL["QuickLook"] .. " 2",
            ...
            dewdrop:AddLine("text", AL["QuickLook"] .. " 3",
            ...
            dewdrop:AddLine("text", AL["QuickLook"] .. " 4",
...
function AtlasLoot_RefreshQuickLookButtons()
    ...
    while i < 5 do
        if ((not AtlasLootCharDB["QuickLooks"][i]) or (not AtlasLootCharDB["QuickLooks"][i][1])) or (AtlasLootCharDB["QuickLooks"][i][1] == nil) then
            dwGetglobal("AtlasLootPanel_Preset" .. i):Disable();
            dwGetglobal("AtlasLootDefaultFrame_Preset" .. i):Disable();
        else
            dwGetglobal("AtlasLootPanel_Preset" .. i):Enable();
            dwGetglobal("AtlasLootDefaultFrame_Preset" .. i):Enable();
        end
        i = i + 1;
    end
end
...
function AtlasLoot_QueryLootPage()
    ...
    while i < 31 do
        ...
        if now - querytime > 0.03 then
            querytime = GetTime();
            button = dwGetglobal("AtlasLootItem_" .. i);
...
function AtlasLoot_AddTooltip(frameb, tooltiptext)
    ...
    local frame = dwGetglobal(frameb)
