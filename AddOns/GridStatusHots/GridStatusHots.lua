...
spellNameCache.GlyphofFlashofLight = GetSpellInfo(54957);
...
GridStatusHots.defaultDB = {
	...
	alert_goflight = {
		text = L["Buff: My Glyph of Flash of Light"],
		desc = L["Buff: My Glyph of Flash of Light"],
		enable = true,
		totshow = true,
		priority = 98,
		range = false,
...
local goflight_hotcolors = {
	["threshold2"] = {
		...
		step = .5,
		get = function ()
			return GridStatusHots.db.profile.alert_goflight.threshold2
		end,
		set = function (v)
			GridStatusHots.db.profile.alert_goflight.threshold2 = v
		end,
	},
	["color2"] = {
		type = "color",
		...
		hasAlpha = true,
		get = function ()
			local color = GridStatusHots.db.profile.alert_goflight.color2
			return color.r, color.g, color.b, color.a
		end,
		set = function (r, g, b, a)
			local color = GridStatusHots.db.profile.alert_goflight.color2
	...
	["threshold3"] = {
		...
		step = .5,
		get = function ()
			return GridStatusHots.db.profile.alert_goflight.threshold3
		end,
		set = function (v)
			GridStatusHots.db.profile.alert_goflight.threshold3 = v
		end,
	},
	["color3"] = {
		type = "color",
		...
		hasAlpha = true,
		get = function ()
			local color = GridStatusHots.db.profile.alert_goflight.color3
			return color.r, color.g, color.b, color.a
		end,
		set = function (r, g, b, a)
			local color = GridStatusHots.db.profile.alert_goflight.color3
	...
	["totshow"] = {
		...
		name = L["Show HoT-Counter"],
		desc = L["Check, if you want to see the total of HoTs behind the countdown of your HoT(i.e. 13-5)"],
		get = function () return GridStatusHots.db.profile.alert_goflight.totshow end,
		set = function (arg)
			GridStatusHots.db.profile.alert_goflight.totshow = arg
		end,
	},
}
...
function GridStatusHots:OnInitialize()
	...
	if englishClass == "DRUID" then
		...
		self:RegisterStatus("alert_lifebl_stack", L["Buff: Lifebloom Stack Colored"], lifebl_hotcolors_stack)
	...
	if englishClass == "PALADIN" then
		...
		self:RegisterStatus("alert_goflight", L["Buff: My Glyph of Flash of Light"], goflight_hotcolors)
...
local function updateUnit(self, gridid, unitid)
		...
		local retime,rjtime,rgtime,lbtime,wgtime,sltime,gltime,ritime,pomtime,boltime,eatime,gratime
		...
		for i=1,999 do
			...
			if bname == spellNameCache["Renew"] then
				...
				if (bismine and btime) then retime = btime end
			elseif bname == spellNameCache["Regrowth"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then rgtime = btime end
			elseif bname == spellNameCache["Rejuvenation"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then rjtime = btime end
			elseif bname == spellNameCache["Lifebloom"] then
				...
				if (bismine and btime) then
					lbtime = btime
					lbstack = bcount
				end
			elseif bname == spellNameCache["WildGrowth"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then wgtime = btime end
			elseif bname == spellNameCache["SheathofLight"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then sltime = btime end
			elseif bname == spellNameCache["GlyphofFlashofLight"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then gltime = btime end
			elseif bname == spellNameCache["Riptide"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then ritime = btime end
			...
			elseif bname == spellNameCache["BeaconofLight"] then
				if (bismine and btime) then boltime = btime end
			elseif bname == spellNameCache["Earthliving"] then
				total_hots = total_hots + 1;
				if (bismine and btime) then eatime = btime end
		...
		if gltime then
			local settings = self.db.profile.alert_goflight
			local hotcolor = settings.color
			if gltime <= settings.threshold2 then hotcolor = settings.color2 end
			if gltime <= settings.threshold3 then hotcolor = settings.color3 end
			if settings.totshow then
				self.core:SendStatusGained(gridid, "alert_goflight",
					settings.priority,
					(settings.range and 40),
					hotcolor,
					tostring(math.floor(gltime).."-"..tostring(total_hots))
				)
			else
				self.core:SendStatusGained(gridid, "alert_goflight",
					settings.priority,
					(settings.range and 40),
					hotcolor,
					tostring(math.floor(gltime))
				)
			end
		else
			if self.core:GetCachedStatus(gridid, "alert_goflight") then self.core:SendStatusLost(gridid, "alert_goflight") end
		end
