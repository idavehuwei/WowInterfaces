...
-- 版权所有 (C) duowan.com
--======================================
local L = LibStub("AceLocale-3.0"):GetLocale("Duowan", true);
...
local LoadOnDemandAddOns = {
	...
	["Postal"] ="MAILMOD",
	["SexyMap"] ="MiniMapMod",
	["Cartographer_QuestInfo"] ="QuestMod",
	["DBM-Core"] ="DBMMod",
...
function dwModeButtonUpdate()
	...
	for i=1, DUOWAN_CONFIGBUTTONCOUNT do
		...
		if (modButton.module and modButton.module == dwCurentMod) then
			...
			getglobal("DuowanConfigFrameModButton" .. i.."UnChecked"):Hide();
			getglobal("DuowanConfigFrameModButton" .. i.."Checked"):Show();

			modButton:SetChecked(1);
		else
			getglobal("DuowanConfigFrameModButton" .. i.."UnChecked"):Show();
			getglobal("DuowanConfigFrameModButton" .. i.."Checked"):Hide();
...
function dwConfigFrame_OnLoad(self)
	...
	--MovieFrame:UnregisterEvent("PLAY_MOVIE");
end
...
function dwConfigFrame_OnEvent(self, event, addon)
	if (event == "ADDON_LOADED") then
		if (isDuowanAddOn(addon) and LoadOnDemandAddOns[addon]) then
			LoadOnDemandAddOns[addon] = false;	-- modified by dugu@bigfoot 2010-4-6
...
function dwMinimapButton_OnLoad(self)
	...
	local cx, cy = dwRawGetCVar("Duowan", "CX", -48), dwRawGetCVar("Duowan", "CY", -54);
	self:ClearAllPoints();
