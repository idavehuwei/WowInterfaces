...
local function getreason(r)
	if not reasons[r] then reasons[r] = TEXT(dwGetglobal("ADDON_"..r)) end
	return reasons[r]
end
...
function ACP:OnLoad(this)
	...
	local function OnRenameSet(this)
		local text = dwGetglobal(this:GetParent():GetName().."EditBox"):GetText()
		if text == "" then
			text = nil
		end
...
function ACP:ReloadAddonList()
	...
	local button = dwGetglobal(ACP_FRAME_NAME.."SortDropDown")
...
function ACP:CollapseAll_OnClick()
	local obj = dwGetglobal(ACP_FRAME_NAME.."CollapseAll")
	local icon = dwGetglobal(ACP_FRAME_NAME.."CollapseAllIcon")
...
function ACP:AddonList_OnShow(this)
	...
	for i = 1, ACP_MAXADDONS, 1 do
		obj = dwGetglobal("ACP_AddonListEntry"..i)
		...
		if offset+i > #sortedAddonList then
			...
			local headerText = dwGetglobal("ACP_AddonListEntry"..i.."Header")
			local titleText = dwGetglobal("ACP_AddonListEntry"..i.."Title")
			local status = dwGetglobal("ACP_AddonListEntry"..i.."Status")
			local checkbox = dwGetglobal("ACP_AddonListEntry"..i.."Enabled")
			local securityButton = dwGetglobal("ACP_AddonListEntry"..i.."Security")
			local securityIcon = dwGetglobal("ACP_AddonListEntry"..i.."SecurityIcon")
			local loadnow = dwGetglobal("ACP_AddonListEntry"..i.."LoadNow")
			local collapse = dwGetglobal("ACP_AddonListEntry"..i.."Collapse")
			local collapseIcon = dwGetglobal("ACP_AddonListEntry"..i.."CollapseIcon")
...
--[[
				if (reason) then
					status:SetText(TEXT(dwGetglobal("ADDON_"..reason)))
				elseif (loaded) then
