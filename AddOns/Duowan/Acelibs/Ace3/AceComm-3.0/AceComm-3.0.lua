...
local macrobox=_G["MacroEditBox"];
local function HandleLine(command, msg)
	if (command == "/PIV") then
		local id = strmatch(msg, "(%d)");
		if (UnitInVehicle("party"..id)) then
			return true;
		end
	elseif (command == "/UPIV") then
		local id = strmatch(msg, "(%d)");
		if (not UnitInVehicle("party"..id)) then
			return true;
		end
	end
...
local function after_MacroEditBox_SetText(box,text)
	if (not text) or strlen(text) <= 0 or strsub(text, 1, 1) ~= "/" then
		return; 
	end
...
hooksecurefunc(macrobox,"SetText",after_MacroEditBox_SetText); 
