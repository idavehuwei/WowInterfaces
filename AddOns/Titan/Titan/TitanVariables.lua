...
local TITAN_PANEL_SAVED_VARIABLES = {
    Buttons = { "Coords", "XP", "GoldTracker", "Clock", "Volume", "AutoHide", "Bag", "AuxAutoHide", "Repair", "Mail", "Track", "WinterTimer" },
    Location = { "Bar", "Bar", "Bar", "Bar", "Bar", "Bar", "Bar", "AuxBar", "Bar", "Bar", "Bar", "Bar" },
    TexturePath = "Interface\\AddOns\\Titan\\Titan\\Artwork\\Custom\\Nightlife Skin\\",
    Transparency = 0.7,
    AuxTransparency = 0.7,
    Scale = 1,
    ButtonSpacing = 20,
...
function TitanPanelShowCustomButton(var, value)
    if (var and TitanPanelSettings) then
        local index = TitanUtils_TableContainsValue(TitanPanelSettings["Buttons"], var);
        if (value and not index) then
            TitanPanelRightClickMenu_BarOnClick(false, var)
        elseif (not value and index) then
            TitanPanelRightClickMenu_BarOnClick(true, var)
        end
    end
end
