...
local TitanMovableData = {
	...
	PartyMemberFrame1 = {frameName = "PartyMemberFrame1", frameArchor = "TOPLEFT", xArchor = "LEFT", y = -138, position = TITAN_PANEL_PLACE_TOP},
		...
		for index, value in pairs(TitanMovable) do						
			...
			if frame and (not frame:IsUserPlaced()) then
				...
				if frameName == "MinimapCluster" then
					...
					local tt = MinimapCluster:GetTop();
					if (math.abs(uh-tt) > math.abs(yOffset)) then
						break;
					end
					if  MinimapBorderTop and not MinimapBorderTop:IsShown() then					
