...
EA_PositionTarxOffset = 136;
EA_PositionTarxOffset = -35;
...
function EventAlert_OnEvent(self, event, ...)
	...
	if (event == "ADDON_LOADED") then
		if (arg1 == "EventAlert") then
            ...
            if EA_Position.Tar_xOffset == nil then EA_Position.Tar_xOffset = EA_PositionTarxOffset end
            if EA_Position.Tar_yOffset == nil then EA_Position.Tar_yOffset = EA_PositionTaryOffset end
	...
	if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
		...
		local EA_spellID = arg9;
		if EA_eventType:match("^SPELL_.+$") or EA_eventType:match("^RANGE_.+$") then
			if (arg7 == UnitName("player") or arg7 == UnitName("target")) then
				if arg7 == nil then arg7 = "nil" end
				EA_spellID = tonumber(EA_spellID);
				if (EA_SpellID_Other[EA_spellID]) then
					EA_SpellID_Other[EA_spellID] = nil;
					local name, rank = GetSpellInfo(EA_SpellID);
					if (not rank) then rank = "" end
					if (not EA_SpellID_DB[name..rank]) then
						EA_SpellID_DB[name..rank] = EA_spellID;
					end					
				end
			end
		end
...
function EventAlert_Buffs_Update(unit)
	...
	for i=1,40 do
		...
		--[[local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster = UnitBuff("player", i);
		local spellId = nil;
		if (name == EA_SpellName_48517) then
			...
			spellId = EA_GetSpellID(name, rank);
		end
		if (name) then
			print(name, spellID, ":", spellId);
		end
		]]
	...
	for i=41,80 do
		...
		--name, rank, icon, count, debuffType, duration, expirationTime, unitCaster = UnitDebuff("player", i-40);
		--local spellId = EA_GetSpellID(name, rank);
	...
	-- 清理已经消失的BUFF	
	for i, v1 in pairs(EA_CurrentBuffs) do
		if(not EA_AltItems[EA_playerClass][v1]) then
			local v2 = table.foreach(buffsCurrent, function(k, v2)
				if (v1 == v2) then
					return v2;
				end
			end);
			...
			-- Buff dropped
			if not v2 and v1 ~= 58597 then
				table.insert(buffsToDelete, v1);
			end
		end
	end

	for i, v in pairs(buffsToDelete) do
		EventAlert_Buff_Dropped(v);
	end

	-- 显示新发现的BUFF
	for i, v1 in pairs(buffsCurrent) do
		local v2 = table.foreach(EA_CurrentBuffs, function (k, v2)
			if (v1 == v2) then
				return v2;
			end
		end);
...
function EA_GetSpellID(spellName, spellRank)
	local spellId = nil;
	if spellName ~= nil then
		-- DEFAULT_CHAT_FRAME:AddMessage("spellName="..spellName.." / spellRank="..spellRank);
		if spellRank == nil then spellRank = "" end
		spellId = EA_SpellID_DB[spellName..spellRank];
	end
