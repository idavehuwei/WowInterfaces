...
function RatingSummary_CreateMainFrame()
	...
	for i = 1, 5 do
		...
		frame["button"..i]:SetPoint("TOPRIGHT", getglobal("RatingSummaryRESbutton" .. (i - 1)) or frame, "BOTTOMRIGHT", 0, -1);
	end
	--base effect
...
function RatingSummary_Sum(inspecting)
	...
	for i=1, 18 ,1 do
		if i~=4 then
			local slotButton = getglobal( (inspecting and "Inspect" or "Character")..slots[i].."Slot");
			local slotBorder = getglobal(slotButton:GetName()):GetNormalTexture();
