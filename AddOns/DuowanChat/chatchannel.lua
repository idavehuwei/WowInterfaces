	...
	for i=1,NUM_CHAT_WINDOWS+1 do
		...
		if(tmp)then
			if(pageID == _G["ChatFrame"..i.."Tab"]:GetText())then
				curTab = dwGetglobal("ChatFrame"..i.."Tab");
				curFrame = dwGetglobal("ChatFrame"..i);
				break;
			end
		end
	end
...
local function FlashChannel(chatFrame, event, message, ...)
	local tab = dwGetglobal(chatFrame:GetName().."Tab");
	local channelid;
...
function ChatChannel:FCF_Close(frame, callback)	
	...
	if(callback)then
		tab = dwGetglobal(callback:GetName().."Tab");
	...
	if(frame)then
		tab = dwGetglobal(frame:GetName().."Tab");
		if(tab)then			
