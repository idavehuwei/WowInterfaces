...
--notes
local revision = tonumber(("$Revision: 2351 $"):match("%d+"));
local Mapster = LibStub("AceAddon-3.0"):GetAddon("Mapster");
local L = LibStub("AceLocale-3.0"):GetLocale("Mapster");
local MODNAME = "Notes"
Mapster_Notes = Mapster:NewModule(MODNAME, "AceConsole-3.0", "AceHook-3.0", "AceEvent-3.0", "AceTimer-3.0");
...
function Mapster_Notes:OnInitialize()
	...
	if path:find("Mapster\\") then
		path = [[Interface\AddOns\Mapster\Arrow]];
	else
...
--	Mapster:RegisterModuleOptions(MODNAME, getOptions, "Notes");
...
function Mapster_Notes:WorldMapButton_OnClick(button, mouseButton)
	if mouseButton ~= "RightButton" or (not IsControlKeyDown() and not IsAltKeyDown()) then
...
local isOpen = false
function Mapster_Notes:WorldMapUnit_OnEnter(this)
	...
	local function newpoi(minimap)
		...
		if not OnClick then
			...
			OnClick = function(self, button)
				...
				if button == "LeftButton" then
					if Mapster_Notes.handlers[self.creator] and type(Mapster_Notes.handlers[self.creator].OnNoteClick) == "function" then
						local data = getrawpoi(self.zone, self.id, self.creator);
						if data then
							--Mapster_Notes.handlers[self.creator]:OnNoteClick(self.zone, self.id, data)
						end
					end
...
function Mapster_Notes:SetNote(zone, x, y, icon, creator, ...)
	...
	local zoneData = usingDB and Mapster_Notes.externalDBs[creator][zone] or self.db.char.pois[zone]
