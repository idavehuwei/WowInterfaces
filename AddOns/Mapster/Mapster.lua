...
local Mapster = LibStub("AceAddon-3.0"):NewAddon("Mapster", "AceEvent-3.0", "AceHook-3.0","AceTimer-3.0")
...
function Mapster:OnInitialize()
	self.db = LibStub("AceDB-3.0"):New("MapsterDB3", defaults, true)
	db_ = self.db.profile
...
function Mapster:OnEnable()
	...
	self:SecureHook("WatchFrameLinkButtonTemplate_OnLeftClick",function(frame,button) 
		if frame.type == "QUEST" then
			if  IsModifiedClick("QUESTWATCHTOGGLE")  then return end
			self:ScheduleTimer(
				function ()
					if frame.index then
						QuestLog_OpenToQuest(GetQuestIndexForWatch(frame.index),true)
					end
				end,
				0.2)
		end
