...
function Mapster:SetupOptions()
    ...
    -- setup options table
    LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable(L["Mapster"], getOptions)
    self.optionsFrames.Mapster = LibStub("AceConfigDialog-3.0"):AddToBlizOptions(L["Mapster"], nil, nil, "general")

    self:RegisterModuleOptions("Profiles", LibStub("AceDBOptions-3.0"):GetOptionsTable(self.db), L["Profiles"])

    LibStub("AceConsole-3.0"):RegisterChatCommand(L["Mapster"], optFunc)
end

function Mapster:RegisterModuleOptions(name, optionTbl, displayName)
    moduleOptions[name] = optionTbl
    self.optionsFrames[name] = LibStub("AceConfigDialog-3.0"):AddToBlizOptions(L["Mapster"], displayName, L["Mapster"], name)
end
