...
local Revision = ("$Revision: 52 $"):sub(12, -3)
...
do
    ...
    mainframe:SetScript("OnEvent", function(self, event, ...)
        ...
        elseif settings.enabled and event == "COMBAT_LOG_EVENT_UNFILTERED" and
                (select(2, ...) == "SPELL_CAST_SUCCESS" or
                 select(2, ...) == "SPELL_RESURRECT" or
                 select(2, ...) == "SPELL_HEAL" or
                 select(2, ...) == "SPELL_AURA_APPLIED" or
                 select(2, ...) == "SPELL_AURA_REFRESH") then
