...
function DCT_Opsave_OnShow()	
	...
	dwGetglobal("DCTSaveOptionEditLabel"):SetText(DCT_TEXT_OPSAVE_EDIT)
end
...
function DCT_Opsave_CheckModeProfile(profiletable)
	...
	for k,v in pairs(profiletable) do
		if not DuowanAddon_DCTOP_SAVE[k] then
			profiletable[k] = nil
		end
	end
end
...
function DCT_Opsave_Load()
	...
	if DuowanAddon_DCTOP_SAVE[curName] then
		DCT_Config_Load(DuowanAddon_DCTOP_SAVE[UIDropDownMenu_GetSelectedName(DCTSaveOptionSaveListDDC)])
...
function DCT_Opsave_Save()
	...
	DuowanAddon_DCTOP_SAVE[curName] = nil
	DuowanAddon_DCTOP_SAVE[curName] = DCT_clone(DCT_Player)
...
function DCT_Opsave_Add()
	...
	if string.len(saveName) > 0 and not DuowanAddon_DCTOP_SAVE[saveName] then		
		DuowanAddon_DCTOP_SAVE[saveName] = DCT_clone(DCT_Player)	
		DCTSaveOption:Hide()
		DCTSaveOption:Show()
...
function DCT_Opsave_Delete()
	...
	if string.len(curName) > 0 and DuowanAddon_DCTOP_SAVE[curName] then
		DuowanAddon_DCTOP_SAVE[curName] = nil;
		DCTSaveOption:Hide()
		DCTSaveOption:Show()
...
function DCT_Opsave_SaveListDDC_Initialize()
	local i;
	local buttonC = 0
	for key,val in pairs(DuowanAddon_DCTOP_SAVE) do
		info = {
			text = key;
...
function DCT_Opsave_SaveListDDC_OnShow()
	UIDropDownMenu_Initialize(this, DCT_Opsave_SaveListDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, 1)
	local id = 0
	for key,val in pairs(DuowanAddon_DCTOP_SAVE) do	
		id = id + 1
		if id == 1 then
			UIDropDownMenu_SetSelectedName(this, key)
		end		
	end
	if id == 0 then
		UIDropDownMenu_SetSelectedName(this, DCT_TEXT_OPSAVE_NONE)
	end
	UIDropDownMenu_SetWidth(this,180)
	dwGetglobal(this:GetName().."Label"):SetText(DCT_TEXT_OPSAVE_SAVELISTDDC)
end
function DCT_Opsave_SaveListDDC_OnClick()
...
function DCT_Opsave_PreSetAniListDDC_OnShow()
	local sel = 1
	...
	UIDropDownMenu_Initialize(this, DCT_Opsave_PreSetAniListDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, sel)
	UIDropDownMenu_SetWidth(this,180)
	dwGetglobal(this:GetName().."Label"):SetText(DCT_TEXT_OPSAVE_PRESETANILISTDDC)
end
function DCT_Opsave_PreSetAniListDDC_OnClick()
	local thisID = this:GetID()	
...
function DCT_Opsave_PreSetTextListDDC_OnShow()
	local sel = 1
	...
	UIDropDownMenu_Initialize(this, DCT_Opsave_PreSetTextListDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, sel)
	UIDropDownMenu_SetWidth(this,180)
	dwGetglobal(this:GetName().."Label"):SetText(DCT_TEXT_OPSAVE_PRESETTEXTLISTDDC)
end
function DCT_Opsave_PreSetTextListDDC_OnClick()
	local thisID = this:GetID()	
...
function DCT_Config_LoadPreSetAni(preId)
	...
	if DCT_PRESET_ANI[preId] then	
		...
		if DCT_PRESET_ANI[preId]["DCT_BASE_CONFIG"] then
			local tmp = DCT_clone(dwGetglobal(DCT_PRESET_ANI[preId]["DCT_BASE_CONFIG"]))			
