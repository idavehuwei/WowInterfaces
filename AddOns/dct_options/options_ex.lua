...
function DCT_Opex_OnShow()
	...
	if DCT_CurrentEditEventName then
		...
		for i = 1,2 do
			dwGetglobal("DCTEventShowEdit"..i):Hide()
		end
		i = 1
		for i = 1,c do
			if c > 1 then
				...
				dwGetglobal("DCTEventShowEdit"..i.."Label"):SetText(txt..":   "..DCT_Opex_EditHelp)
			else
				dwGetglobal("DCTEventShowEdit"..i.."Label"):SetText(DCT_Opex_EditHelp)
			end
			dwGetglobal("DCTEventShowEdit"..i):SetText(tmp.show[i])
			dwGetglobal("DCTEventShowEdit"..i):SetCursorPosition(1)
			dwGetglobal("DCTEventShowEdit"..i):Show()
		end
		i = 1
		for i = 1,2 do
			cp = dwGetglobal("DCTEventOptionsExtra_ColorPickerEx"..i)
			cp.swatchFunc = function(x) DCT_Opex_SetColor(dwGetglobal("DCTEventOptionsExtra_ColorPickerEx"..i):GetName()) end;
			cp.cancelFunc = function(x) DCT_Open_CancelColor(dwGetglobal("DCTEventOptionsExtra_ColorPickerEx"..i):GetName(),x) end;
			cp.r = tmp.colorEx[i][1]
			cp.g = tmp.colorEx[i][2]
			cp.b = tmp.colorEx[i][3]
			dwGetglobal("DCTEventOptionsExtra_ColorPickerEx"..i.."_Text"):SetText(DCT_TEXT_OPEX_COLOR[i])
			dwGetglobal("DCTEventOptionsExtra_ColorPickerEx"..i.."_ColorSwatchNormalTexture"):SetVertexColor(cp.r,cp.g,cp.b)
		end		
	end
	...
	for key,val in pairs(DCT_Spec_default) do
		if DCT_opex_saveP[DCT_CurrentEditEventName][key] == nil then
			dwGetglobal("DCTOPEXICB_"..key):SetChecked(true)
		else
			dwGetglobal("DCTOPEXICB_"..key):SetChecked(false)
		end
		dwGetglobal("DCTOPEXICB_"..key.."Text"):SetText(DCT_TEXT_OPEX_CHECK)
	end
	...
	for i = 1,10 do
		k = nil
		obj = dwGetglobal("DCTEventOptionsExtraHelp"..i)
		if i <= hc then
...
function DCT_Opex_EditPress()
	local c = string.len(this:GetName())
	DCT_opex_saveP[DCT_CurrentEditEventName].show[tonumber(string.sub(this:GetName(),c,c))] = this:GetText()
end

function DCT_Opex_SoundEditPress()
	local c = string.len(this:GetName())
	DCT_opex_saveP[DCT_CurrentEditEventName].sound = this:GetText()
end
...
function DCT_Opex_SetColor(name)
	...
	local swatch,frame;
	swatch = dwGetglobal(name.."_ColorSwatchNormalTexture");
	frame = dwGetglobal(name);
...
function DCT_Opex_FontEnDDC_OnShow()
	UIDropDownMenu_Initialize(this, DCT_Opex_FontEnDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, DCT_opex_saveP[DCT_CurrentEditEventName].fEn)
...
function DCT_Opex_FontChDDC_OnShow()
	UIDropDownMenu_Initialize(this, DCT_Opex_FontChDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, DCT_opex_saveP[DCT_CurrentEditEventName].fCh)
	UIDropDownMenu_SetWidth(this,150)
...
function DCT_Opex_FontEffDDC_OnShow()
	UIDropDownMenu_Initialize(this, DCT_Opex_FontEffDDC_Initialize)
	UIDropDownMenu_SetSelectedID(this, DCT_opex_saveP[DCT_CurrentEditEventName].fEff)
	UIDropDownMenu_SetWidth(this,150)
	dwGetglobal(this:GetName().."Label"):SetText(DCT_TEXT_OPEX_FONTEFFDDC)
end
function DCT_Opex_FontEffDDC_OnClick()
	local thisID = this:GetID()
...
function DCT_Opex_SliderOnValue(objname,value)	
	local c = string.len(this:GetName())
	local f = string.find(this:GetName(),"_")
...
function DCT_Opex_InheritsCheckBoxClick()
