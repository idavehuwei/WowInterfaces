...
function DCT_OptionsGetSaveP(name)
	...
	if DCT_SavePreFixesList[prename] then
		p = dwGetglobal(DCT_SavePreFixesList[prename])
	end
	return p
end
...
function DCT_OptionsButtonClick(bp)
	...
	DCT_Options_Title_Text:SetText( (dwGetglobal(bp:GetName().."_Text")):GetText() )
...
function DCT_OptionsRegisterButton(text,frame,funcOnClick,y,bgw,bgh)
	...
	--obj:SetText(text)
	dwGetglobal(obj:GetName().."_Text"):SetText(text)
...
function DCT_ConfigSlider(obj,curValue,paramP)
	...
	if paramP.title then
		...
		dwGetglobal(objname.."Text"):SetText(obj.titlebak)
	end
	...
	if paramP.minText then
		dwGetglobal(objname.."Low"):SetText(paramP.minText)
	else
		dwGetglobal(objname.."Low"):SetText(paramP.minValue)
	end
	if paramP.maxText then
		dwGetglobal(objname.."High"):SetText(paramP.maxText)
	else
		dwGetglobal(objname.."High"):SetText(paramP.maxValue)
	end

	
	if curValue then
		dwGetglobal(objname.."EditBox"):SetText(curValue)
		obj.missOnValue = true
		obj:SetValue(curValue)
	else
		dwGetglobal(objname.."EditBox"):SetText("")
	end
end
...
function DCT_ConfigColorPicker(objname,saveP)
	...
	frame = dwGetglobal(objname);
	swatch = dwGetglobal(objname.."_ColorSwatchNormalTexture");
...
function DCT_RefreshEventFrames(objname,saveP)
	if saveP == nil then saveP = DCT_Player;end
	local button = dwGetglobal(objname.."_Button_Text")
	local saveName = dwGetglobal(objname).saveName
	local val = saveP[saveName].frame
	
	if val > DCT_Ani_MaxFrame then val = DCT_Ani_MaxFrame;end
	if val < 0 then val = 0;end
	
	dwGetglobal(objname).curFrame = val
	
	if val == 0 or (val > 0 and not DCT_Ani_FrameIsActive(val)) then
		dwGetglobal(objname.."_Text"):SetTextColor(0.4,0.4,0.4)
		button:SetText(DCT_GetEventIDString(val + 1))
	else
		dwGetglobal(objname.."_Text"):SetTextColor(1.0,0.82,0)
	...
	if saveP[saveName].fEn or saveP[saveName].fCh or saveP[saveName].fSizeEn or saveP[saveName].fSizeCh or saveP[saveName].fEff then
		dwGetglobal(objname.."_ButtonEx_Text"):SetText("A")
		dwGetglobal(objname.."_ButtonEx_Text"):SetTextColor(1,1,0)
	else
		dwGetglobal(objname.."_ButtonEx_Text"):SetText("e")
		dwGetglobal(objname.."_ButtonEx_Text"):SetTextColor(1,1,1)
	end
...
function DCT_OptionsFrame_Clear()
	...
	for id = 1,30 do
		local obj = dwGetglobal("DCT_Event"..id);	
		obj:Hide();
	end
	for id = 1,20 do
		local obj = dwGetglobal("DCT_Check"..id);	
		obj:Hide();
	end
	for id = 1,20 do
		dwGetglobal("DCT_ColorPickerEx"..id):Hide()
	end
end
...
function DCT_OptionsFrame_Load(cfg,saveP)
	...
	if cfg.EventFrames then
		for key, value in pairs(cfg.EventFrames) do
			...
			local obj = dwGetglobal(objname);		
			dwGetglobal(objname.."_Button").Func = DCT_Event_Button_Click
			dwGetglobal(objname).tooltipText = value.tooltipText;
			obj.saveName = key;
			obj:SetPoint("TOPLEFT", "DCT_Options", "TOPLEFT", value.x, value.y);
			dwGetglobal(objname.."_Text"):SetText(value.title);
	...
	if cfg.CheckButtons then
		for key, value in pairs(cfg.CheckButtons) do
			...
			obj.saveName = key
			obj:SetPoint("TOPLEFT", "DCT_Options", "TOPLEFT", value.x, value.y)
			dwGetglobal(objname.."Text"):SetText(value.title)
	...
	if cfg.ColorPickerExs then
		for key, value in pairs(cfg.ColorPickerExs) do
			...
			local colorObj = dwGetglobal(objname.."_ColorSwatch")
			local colorBGObj = dwGetglobal(objname.."_ColorSwatch_SwatchBg")
			size = 18
			if value.size then size = value.size;end
			...
			if value.title then dwGetglobal(objname.."_Text"):SetText(value.title);end
			DCT_ConfigColorPicker(objname);	
...
function DCT_OptionsCheckButtonOnClick(name,checkboxname)	
	local button = dwGetglobal(checkboxname);
...
function DCT_Event_Button_Click(name,button)
	for name in string.gmatch( name, "(.+)_Button" ) do
		local saveName = dwGetglobal(name).saveName
		...
		dwGetglobal(name.."_Button_Text"):SetText(txt);		
...
function DCTSlider_EditBoxEnterPressed(newVal)
	local tmin,tmax = (this:GetParent()):GetMinMaxValues();
	if newVal < tmin then newVal = tmin;end
	if newVal > tmax then newVal = tmax;end
	(this:GetParent()):SetValue(newVal);
	this:ClearFocus();
end

----------------------
--Open the color selector using show/hide
function DCT_OpenColorPicker(button)
	CloseMenus();
	if ( not button ) then
		button = this;
	end
...
function DCT_ShowBG(id,x,y,w,h)
	local obj = dwGetglobal("DCT_Options_BG"..id);
	obj:Show();
