...
local TT_DefaultConfig = {
	...
	showRealm = "show",
	showTarget = "last",
	targetYouText = TTCL_targetYouText,
	showTargetedBy = true,
	...
	classification_elite = TTCL_Elite,
	classification_worldboss = TTCL_Boss,
	classification_rare = TTCL_Rare,
	classification_rareelite = TTCL_Rareelite,
...
function tt:VARIABLES_LOADED(event)
	...
	if (cfg.left and cfg.top) then
		self:ClearAllPoints();
		self:SetPoint("BOTTOMRIGHT",UIParent,"BOTTOMRIGHT",cfg.left,cfg.top);
	else
		self:Show();
...
local function ModifyUnitTooltip()
	...
	if (u.isPlayer) then
		...
		if (cfg.showStatus) then
			local status = (not UnitIsConnected(unit) and TTCL_DC) or (UnitIsAFK(unit) and TTCL_AFK) or (UnitIsDND(unit) and TTCL_DND);
			if (status) then
	...
	if (cfg.showTarget ~= "none") then
		...
		if (target) and (target ~= UNKNOWN and target ~= "" or UnitExists(targetUnit)) then
			...
			elseif (cfg.showTarget == "last") then
				lineInfo[#lineInfo + 1] = TTCL_Targeting;
...
local function AddTargetedBy()
	...
	if (numParty > 0 or numRaid > 0) then
		...
		if (#targetedList > 0) then
			...
			line:SetFormattedText(TTCL_Targetedby.."(|cffffffff%d|r): %s",(#targetedList + 1) / 3,table.concat(targetedList));
			wipe(targetedList);
		end
	end
end
