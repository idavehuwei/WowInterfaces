...
function TgFocusFrame_SetScale(scale)
    if InCombatLockdown() then
        print('TgFocusFrame: You cannot change scale while in combat.');
        return;
    end;
...
function TgFocusFrame_HideWhenDead(toggle)
    ...
    if toggle then
        if InCombatLockdown() then
            print('TgFocusFrame: You cannot toggle hidewhendead while in combat.');
            return;
        end;
...
function TgFocusFrameHealthBarText_UpdateTextString(textStatusBar)
    ...
    if ( valueMax > 0 ) then
        ...
        local showCurr=curHP;
        local showMax=maxHP;

        if maxHP > 1000000 then
            showMax = math.floor(maxHP/10000);
            showMax = string.format(EUF_TEXT_10KTAG, showMax);
        end;

        if curHP > 1000000 then
            showCurr = math.floor(curHP/10000);
