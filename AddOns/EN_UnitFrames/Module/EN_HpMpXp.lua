...
function EUF_HpMpXp_OnLoad(self)
	...
	TargetFrameManaBarText:SetAlpha(0);
	TargetFrameHealthBarText:SetAlpha(0);
...
function EUF_HP_Update(unit)
	...
	if (string.find(unit, "^party%d$") or unit == "pet") then
...
function EUF_PlayerFrameXp_Update()
	...
	-- 显示声望
    if ReputationWatchBar:IsVisible() and name then
       if(EUF_CurrentOptions) then
...
function EUF_XPBarToggle(switch)
	if (switch) then
		...
		EUF_ObjectDisplay_Update(EUF_PlayerFrameXP, true);

	else
		EUF_CurrentOptions["PLAYERXP"] = 0;
		EUF_ObjectDisplay_Update(EUF_PlayerFrameXP, false);
	end
end
