...
-- Saved variables
TgFocusFrameOptions = FocusFrameOptions or {};
...
local function TgFocusFrame_SlashCommand(msg)
  ...
  elseif cmd == "lock" then
    TgFocusFrameOptions.lockpos = true;
  elseif cmd == "unlock" then
    TgFocusFrameOptions.lockpos = nil;
...
function TgFocusFrame_Help()
  ...
  DEFAULT_CHAT_FRAME:AddMessage('/focusframe hidewhendead : Hide when focused enemy target is dead. ['..((TgFocusFrameOptions.hidewhendead and 'ON') or 'OFF')..']');
end
...
function TgFocusFrame_SetScale(scale)
  ...
  if scalenum and scalenum <= 10 then
    TgFocusFrameOptions.scale = scalenum;
...
function TgFocusFrame_HideWhenDead(toggle)
	if TgFocusFrameOptions.hidewhendead == nil then
		-- Default is ON
		TgFocusFrameOptions.hidewhendead = true;
	end
	if toggle then
		if InCombatLockdown() then
		...
		if TgFocusFrameOptions.hidewhendead then
			TgFocusFrameOptions.hidewhendead = false;
...
function TgFocusFrame_Toggle(switch)
	local self = TgFocusFrame;
	if (switch) then
...
function TgFocusFrame_OnEvent(self, event, ...)	
    ...
    elseif ( event == "VARIABLES_LOADED" ) then
        TgFocusFrameOptions.scale = TgFocusFrameOptions.scale or 1;
		TgFocusFrame_SetScale(TgFocusFrameOptions.scale);
	...
	for i=1, l.MAX_FOCUS_BUFFS do
		...
		if ( not button ) then
			if ( not icon ) then
				...
				button = CreateFrame("Button", "TgFocusFrameBuff"..i, TgFocusFrame, "FocusBuffButtonTemplate");
			end
		end
	...
	for i=1, l.MAX_FOCUS_DEBUFFS do
		...
		if ( not button ) then
			if ( not icon ) then
				...
				button = CreateFrame("Button", "TgFocusFrameDebuff"..i, TgFocusFrame, "FocusDebuffButtonTemplate");
...
function TargetofTgFocus_Toggle(switch)
	local self = TargetofTgFocusFrame;
	if (switch) then
		RegisterUnitWatch(self);  
...
function TargetofTgFocus_Update(self)
	if ( TargetofTgFocusFrame:IsShown() ) then
		...
		RefreshDebuffs(TargetofTgFocusFrame, "focustarget");

		if (UnitIsPlayer("focustarget")) then
			local _,enClass = UnitClass("focustarget");
			local color = RAID_CLASS_COLORS[enClass];
			TargetofTgFocusName:SetTextColor(color.r or 1, color.g or 0.8, color.b or 0);
		else
			TargetofTgFocusName:SetTextColor(1, 0.8, 0);
		end
	end
end
...
function TargetofTgFocusHealthCheck(self)
	if (UnitExists("focustarget")) then
		if (UnitIsDead("focustarget") or UnitIsGhost("focustarget") ) then
			...
			local unitCurrHP = UnitHealth("focustarget");
			local unitHPMax = UnitHealthMax("focustarget");
			TargetofTgFocusFrame.unitHPPercent = unitCurrHP / unitHPMax;
			TargetofTgFocusHPText:SetText(string.format("%d%%", TargetofTgFocusFrame.unitHPPercent * 100));
			TargetofTgFocusHPText:Show();

			if (UnitIsPlayer("focustarget")) then
				if ((TargetofTgFocusFrame.unitHPPercent > 0) and (TargetofTgFocusFrame.unitHPPercent <= 0.2)) then
...
function TgFocusFrame_OnDragStart(self)
	if (not TgFocusFrameOptions.lockpos) then
