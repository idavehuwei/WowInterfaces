...
function EN_TargetTargetTarget_OnLoad(self)
	...
	local text = TargetofTargetTextureFrame:CreateFontString("TargetofTargetFrameHPText", "OVERLAY" , "GameFontNormalSmall")
	text:SetPoint("CENTER", TargetofTargetFrame, "CENTER", 20, 1);
...
function EN_TargetTargetTarget_Update(self, elapsed)
	...
	if UnitExists("target") and UnitExists("targettarget") and UnitIsPlayer("targettarget") then		
		...
		TargetofTargetName:SetTextColor(color.r or 1, color.g or 0.8, color.b or 0);		
	else
		TargetofTargetName:SetTextColor(1, 0.8, 0);
	end
...
function EN_TargetTargetTargetHealthCheck()
	...
	if (UnitExists("targettarget")) then
		if (UnitIsDead("targettarget") or UnitIsGhost("targettarget") ) then
			...
			TargetofTargetFrameHPText:SetText(string.format("%d%%", TargetofTargetFrame.unitHPPercent * 100));
			TargetofTargetFrameHPText:Show();			
		end
	end
