...
function Atr_BuildHints (itemLink, itemName)
	...
	if (itemLink) then
		...
		local id = ZC_ItemIDfromLink (itemLink);
...
function Atr_ShowHints ()
	...
	Atr_Col3_Heading:SetText ("Source");

	local currentPane = Atr_GetCurrentPane();

	currentPane.hints = Atr_BuildHints (currentPane.itemLink, currentPane.itemName);
	
	local numrows = #currentPane.hints;
	...
	for line = 1,12 do
		...
		if (dataOffset <= numrows and currentPane.hints[dataOffset]) then
			...
			MoneyFrame_Update (lineEntry_item_tag, ZC_round(data.price) );

			local text = data.text;
			if (data.volume) then
				text = text.." (trade volume: "..data.volume..")";
			end
	...
	if (#currentPane.hints and not currentPane.hintsIndex) then
...
function Atr_GetNextDustIntoCache()		-- make sure all the dusts and essences are in the local cache
	...
	if (itemLink == nil and dustCacheState == 0) then
		dustCacheState = 1;
		if (Atr_IsDev) then
			ZC_redmsg ("pulling "..itemString.." into the local cache");
		end
		MyScanningTooltip:SetHyperlink(itemString);
	end

	if (itemLink) then
--		ZC_yellowmsg (itemName.." now in local cache");
		dustCacheState = 0;
...
local function Atr_GetDEitemName (itemID)
	...
	if (deItemNames[itemID] == nil) then
		...
		if (itemName == nil) then
			ZC_redmsg ("defaulting to english DE mat name: "..engDEnames [itemID]);
...
function Atr_InitDETable()		-- based on table at wowwiki.com/Disenchanting_tables
	...
	DEtableInsert (t, {121, 151,	75, {1,2}, INFINITE_DUST,	22, {1,2}, LESSER_COSMIC,	3, 1, SMALL_DREAM});
	DEtableInsert (t, {152, 200,	75, {2,5}, INFINITE_DUST,	22, {1,2}, GREATER_COSMIC,	3, 1, DREAM_SHARD});
	...
	DEtableInsert (t, {121, 151,	22, {1,2}, INFINITE_DUST,	75, {1,2}, LESSER_COSMIC,	3, 1, SMALL_DREAM});
	DEtableInsert (t, {152, 200,	22, {2,5}, INFINITE_DUST,	75, {1,2}, GREATER_COSMIC,	3, 1, DREAM_SHARD});
...
--	DEtableInsert (t, {61, 80,  FILLED IN BELOW
	...
	deTable[deKey(WEAPON, EPIC)] = ZC_CopyDeep (deTable[deKey(ARMOR, EPIC)]);	-- copy it this time because of differences
...
local function Atr_AddDEDetailsToTip (tip, itemType, itemRarity, itemLevel, DEreqLevel)
	...
	tip:AddLine ("  |cFFAAAAFFRequired DE skill level: "..DEreqLevel);
end
...
function Atr_DumpDETable (itemType, itemRarity)
	...
	if (t) then
		...
		for n = 1, #t do
			...
			ZC_pinkmsg ("iLvl: "..ta[1].."-"..ta[2]);
			
			for x = 3,#ta,3 do
				ZC_pinkmsg ("   "..ta[x].."%  "..ta[x+1].."  "..Atr_GetDEitemName(ta[x+2]).."  ("..Atr_GetAuctionPrice (Atr_GetDEitemName(ta[x+2]))..")");
			end
		end
	end
...
function Atr_CalcDisenchantPrice (itemType, itemRarity, itemLevel)
	...
	if (Atr_IsWeapon (itemType) or Atr_IsArmor (itemType)) then
		if (itemRarity == UNCOMMON or itemRarity == RARE or itemRarity == EPIC) then
			...
			if (ta) then
				local x;
				for x = 3,#ta,3 do
					local price = Atr_GetAuctionPrice (Atr_GetDEitemName(ta[x+2]));
					if (price) then
...
local function ShowTipWithPricing (tip, link, num)
	...
	local itemID = ZC_ItemIDfromLink (link);
	...
	if (AUCTIONATOR_V_TIPS == 1) then vendorPrice	= Atr_GetVendorPrice (itemID); end;
	...
	if (AUCTIONATOR_V_TIPS == 1 and vendorPrice > 0) then
		...
		tip:AddDoubleLine (ZT("Vendor")..xstring, "|cFFFFFFFF"..ZC_priceToMoneyString (vendorPrice))
	end
	...
	if (AUCTIONATOR_A_TIPS == 1) then
		...
		elseif (auctionPrice ~= nil) then
			tip:AddDoubleLine (ZT("Auction")..xstring, "|cFFFFFFFF"..ZC_priceToMoneyString (auctionPrice));
		else
	...
	if (AUCTIONATOR_D_TIPS == 1 and dePrice ~= nil) then
		if (dePrice > 0) then
			tip:AddDoubleLine (ZT("Disenchant")..xstring, "|cFFFFFFFF"..ZC_priceToMoneyString(dePrice));
		else
