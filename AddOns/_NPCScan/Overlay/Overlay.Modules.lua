...
local function GetEnabled ( Options, Name ) -- Gets enabled state from options table
	return _NPCScan.Options.EnableScan and Options.Modules[ Name ] ~= false; -- Uninitialized (nil) defaults to enabled
end
...
function me.Enable ( Name )
	--if ( not Overlay.Options.Modules[ Name ] or _NPCScan.Options.EnableScan) then
...
function me.Disable ( Name )
	--if ( GetEnabled( Overlay.Options, Name ) or not _NPCScan.Options.EnableScan) then
