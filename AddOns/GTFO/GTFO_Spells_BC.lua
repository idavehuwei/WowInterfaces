--------------------------------------------------------------------------
-- GTFO_Spells_BC.lua 
--------------------------------------------------------------------------
--[[
GTFO Spell List - Burning Crusade
Author: Zensunim of Malygos

Change Log:
	v0.2
		- Added Spells
	v1.0.1
		- Replaced some spell names with spell IDs
	v1.1
		- Replaced some spell names with spell IDs
	v2.5
		- Split spell files into sections
	v2.9.1
		- Added Omor the Unscarred's Treacherous Aura
		- Added Omor the Unscarred's Bane of Treachery
		- Added Broggok's Poison
	v3.0
		- Moved Netherspite's Void Zone (from Generic)
		- Added Shade of Aran's Blizzard
		- Added Nightbane's Charred Earth
	v3.0.1
		- Added Hungarfen's Foul Spores
		- Added Hydromancer Thespia's Lightning Cloud
		- Added Grandmaster Vorpil's Rain of Fire
		- Added Gatewatcher Iron-Hand's Jackhammer
		- Added Lieutenant Drake's Whirlwind
		- Added Captain Skarloc's Consecration
		- Added Rage Winterchill's Death and Decay
		- Added Azgalor's Rain of Fire
		- Added Archimonde's Doomfire		
		- Added Supremus's Molten Flame
		- Added Supremus's Volcanic Geyser
		- Added Ashtongue Elementalist's Rain of Fire
	v3.1
		- Updated Omor the Unscarred's Treacherous Aura to use Friendly Fire
		- Updated Omor the Unscarred's Bane of Treachery to use Friendly Fire
	v3.2
		- Added Sethekk Spirit's Spirit Burst
		- Added Grand Warlock Nethekurse's Dark Spin
		- Added Mother Shahraz's Fatal Attraction
		- Added Lady Vashj's Static Charge
		- Added Shadowmoon Darkcaster's Rain of Fire
		- Added Grand Warlock Nethekurse's Consumption
		- Added Shattered Halls's Flames
		- Added Mechanar Tinkerer's Netherbomb
		- Added Tempest-Forge Destroyer's Charged Fist
		- Added Nethermancer Sepethrea's Raging Flames
		- Added Nethermancer Sepethrea's Inferno
		- Added Zereketh the Unbound's Consumption
		- Added Destroyed Sentinel's Energy Discharge
		- Added Dalliah the Doomsayer's Whirlwind
		- Added Wrath-Scryer Soccothrates's Felfire
		- Added Kael'thas Sunstrider's Burn
		- Added Kael'thas Sunstrider's Arcane Sphere
		- Added Sunseeker Chemist's Vial of Poison
		- Added Sunseeker Gene-Splicer's Death and Decay
		- Added Thorngrin the Tender's Hellfire
	v3.2.1
		- Added Hellfire Warder's Rain of Fire
		- Added Magtheridon's Debris
		- Added Gruul's Cave In
		- Added High King Maulgar's Whirlwind
		- Added Kiggler the Crazed's Arcane Explosion
		- Added Krosh Firehand's Blast Wave		
	v3.2.4
		- Added Netherspite's Perseverence
		- Added Netherspite's Dominance
		- Added Prince Malchezaar's Hellfire Effect
	v3.2.6
		- Moved High King Maulgar's Whirlwind
]]--

GTFO.SpellID["40611"] = {
	--desc = "Blaze (Illidan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["41482"] = {
	--desc = "Blizzard (Illidari Council)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["41481"] = {
	--desc = "Flamestrike (Illidari Council)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["41541"] = {
	--desc = "Consecration (Illidari Council)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["28865"] = {
	--desc = "Consumption (Netherspite)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["31944"] = {
	--desc = "Doomfire (Archimonde, CoT:HS)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["30129"] = {
	--desc = "Charred Earth (Nightbane)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["50746"] = {
	--desc = "Burn (Illidan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["51103"] = {
	--desc = "Frostbomb (Mage-Lord Urom)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["56926"] = {
	--desc = "Thundershock (Jedoga Shadowseeker)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["60029"] = {
	--desc = "Thundershock (Jedoga Shadowseeker - Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["37469"] = {
	--desc = "Poison Cloud (Orc Warlock - Karazhan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["30697"] = {
	--desc = "Treacherous Aura (Omor the Unscarred - Hellfire Ramparts)";
	ignoreSelfInflicted = true;
	sound = 4;
	trivialLevel = 80;
};

GTFO.SpellID["39298"] = {
	--desc = "Bane of Treachery (Omor the Unscarred - Hellfire Ramparts - Heroic)";
	ignoreSelfInflicted = true;
	sound = 4;
	trivialLevel = 80;
};

GTFO.SpellID["30915"] = {
	--desc = "Poison (Broggok - Blood Furnace)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["40657"] = {
	--desc = "Ancient Flames (Terokk)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["30533"] = {
	--desc = "Void Zone (Netherspite - Karazhan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["29951"] = {
	--desc = "Blizzard (Shade of Aran - Karazhan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["30129"] = {
	--desc = "Charred Earth (Nightbane - Karazhan)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["31697"] = {
	--desc = "Foul Spores (Hungarfen - Underbog)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["25033"] = {
	--desc = "Lightning Cloud (Hydromancer Thespia - Steam Vaults)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["33617"] = {
	--desc = "Rain of Fire (Grandmaster Vorpil - Shadow Labyrinth)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["39363"] = {
	--desc = "Rain of Fire (Grandmaster Vorpil - Shadow Labyrinth - Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["35330"] = {
	--desc = "Jackhammer (Gatewatcher Iron-Hand - Mechanar)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["39195"] = {
	--desc = "Jackhammer (Gatewatcher Iron-Hand - Mechanar - Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["31910"] = {
	--desc = "Whirlwind (Lieutenant Drake - CoT:OHF)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["38385"] = {
	--desc = "Consecration (Captain Skarloc - CoT:OHF)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["31258"] = {
	--desc = "Death & Decay (Rage Winterchill - CoT:HS)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["31340"] = {
	--desc = "Rain of Fire (Azgalor - CoT:HS)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["40265"] = {
	--desc = "Molten Flame (Supremus - Black Temple)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["42052"] = {
	--desc = "Volcanic Geyser (Supremus - Black Temple)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["42023"] = {
	--desc = "Rain of Fire (Ashtongue Elementalist - Black Temple)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["32302"] = {
	--desc = "Focus Fire (Shirrak the Dead Watcher - Auchenai Crypts)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["38382"] = {
	--desc = "Focus Fire (Shirrak the Dead Watcher - Auchenai Crypts - Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["24050"] = {
	--desc = "Spirit Burst (Sethekk Spirit - Sethekk Halls)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["30508"] = {
	--desc = "Dark Spin (Grand Warlock Nethekurse - Shattered Halls)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["40871"] = {
	--desc = "Fatal Attraction (Mother Shahraz - Black Temple)";
	sound = 4;
	ignoreSelfInflicted = true;
	trivialLevel = 80;
};

GTFO.SpellID["40871"] = {
	--desc = "Static Charge (Lady Vashj - SSC)";
	sound = 4;
	ignoreSelfInflicted = true;
	trivialLevel = 80;
};

GTFO.SpellID["11990"] = {
	--desc = "Rain of Fire (Shadowmoon Darkcaster - Shattered Halls)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["30979"] = {
	--desc = "Flames (Shattered Halls)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["32251"] = {
	--desc = "Consumption (Grand Warlock Nethekurse - Shattered Halls)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["35057"] = {
	--desc = "Netherbomb (Mechanar Tinkerer - Mechanar)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["38925"] = {
	--desc = "Netherbomb (Mechanar Tinkerer - Mechanar Heroic)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["36583"] = {
	--desc = "Charged Fist (Tempest-Forge Destroyer - Mechanar)";
	sound = 2;
	tankSound = 0;
	trivialLevel = 80;
};

GTFO.SpellID["35312"] = {
	--desc = "Raging Flames (Nethermancer Sepethrea - Mechanar)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["35283"] = {
	--desc = "Inferno (Nethermancer Sepethrea - Mechanar)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["36121"] = {
	--desc = "Consumption (Zereketh the Unbound - Arcatraz)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["36717"] = {
	--desc = "Energy Discharge (Destroyed Sentinel - Arcatraz)";
	sound = 1;
	specificMobs = { 21761 }; -- Destroyed form only
	trivialLevel = 80;
};

GTFO.SpellID["38829"] = {
	--desc = "Energy Discharge (Destroyed Sentinel - Arcatraz Heroic)";
	sound = 1;
	specificMobs = { 21761 }; -- Destroyed form only
	trivialLevel = 80;
};

GTFO.SpellID["36175"] = {
	--desc = "Whirlwind (Dalliah the Doomsayer - Arcatraz)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["35767"] = {
	--desc = "Felfire (Wrath-Scryer Soccothrates - Arcatraz)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["44198"] = {
	--desc = "Burn (Kael'thas Sunstrider - Magisters' Terrace)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["36721"] = {
	--desc = "Burn (Kael'thas Sunstrider - Magisters' Terrace Heroic)";
	sound = 1;
	trivialLevel = 80;
	test = true;
};

GTFO.SpellID["44264"] = {
	--desc = "Arcane Sphere (Kael'thas Sunstrider - Magisters' Terrace)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["34358"] = {
	--desc = "Vial of Poison (Sunseeker Chemist - Botanica)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["39127"] = {
	--desc = "Vial of Poison (Sunseeker Chemist - Botanica Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["34642"] = {
	--desc = "Death and Decay (Sunseeker Gene-Splicer - Botanica)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["39347"] = {
	--desc = "Death and Decay (Sunseeker Gene-Splicer - Botanica Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["34660"] = {
	--desc = "Hellfire (Thorngrin the Tender - Botanica)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["39132"] = {
	--desc = "Hellfire (Thorngrin the Tender - Botanica Heroic)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["34435"] = {
	--desc = "Rain of Fire (Hellfire Warder - Magtheridon's Lair)";
	sound = 2;
	trivialLevel = 80;
};

GTFO.SpellID["36449"] = {
	--desc = "Debris (Magtheridon)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["36240"] = {
	--desc = "Cave In (Gruul)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["33237"] = {
	--desc = "Arcane Explosion (Kiggler the Crazed - Gruul's Lair)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["33061"] = {
	--desc = "Blast Wave (Krosh Firehand - Gruul's Lair)";
	sound = 1;
	trivialLevel = 80;
};

GTFO.SpellID["30421"] = {
	--desc = "Perseverance (Netherspite - Karazhan)";
	sound = 1;
	applicationOnly = true;
	minimumStacks = 30;
	trivialLevel = 80;
};

GTFO.SpellID["30423"] = {
	--desc = "Dominance (Netherspite - Karazhan)";
	sound = 1;
	applicationOnly = true;
	minimumStacks = 30;
	trivialLevel = 80;
};

GTFO.SpellID["30860"] = {
	--desc = "Hellfire Effect (Prince Malchezaar - Karazhan)";
	sound = 1;
};
