            ...
            Duowan_ShowPopRange(arg1,
                ...
                function(value)
                    for i = 1, SPELLTIMER_MAX_UNITFRAME_COUNT do
                        local frame = dwGetglobal("SpellTimerUnitFrame" .. i);
                        if (frame) then
...
function SpellTimer:Toggle(toggle)
    ...
    elseif (not toggle and SpellTimer_Enabled) then
        for i = 1, SPELLTIMER_MAX_SPELLFRAME_COUNT, 1 do
            local frame = dwGetglobal("SpellTimerSpellFrame" .. i);
            if (frame) then
                frame:Hide();
            end
        end
...
function SpellTimer:SyncTime(unit)
    ...
    for i = 1, MAX_TARGET_DEBUFFS do
        ...
        if ((unitCaster == "player" or unitCaster == "pet") and name and duration and duration > 0) then
            if ((SpellTimer_SpellsSaved[name] and SPELL_TIMER_PATTERN_TABLE[SpellTimer_Class][name][1] ~= "buff") or (SpellTimer_Class == "PALADIN" and name:find(SPELL_TIMER_JUDGEMENT))) then
    ...
    -- 寻找unit对应的计时器
    for i = 1, SPELLTIMER_MAX_UNITFRAME_COUNT, 1 do
        local unit_frame = dwGetglobal("SpellTimerUnitFrame" .. i);
    ...
    while (true) do
        ...
        if (name) then
            if (name and duration and duration > 0) then
                if ((SpellTimer_SpellsSaved[name] and (SPELL_TIMER_PATTERN_TABLE[SpellTimer_Class][name][1] == "buff" or SPELL_TIMER_PATTERN_TABLE[SpellTimer_Class][name][1] == "both")) or (SpellTimer_Class == "PALADIN" and name:find(SPELL_TIMER_JUDGEMENT))) then
                    if (not cache[name]) then
...
function SpellTimer_UpdateStatus(frame, guid)
    if (frame and frame.target_guid and frame.target_name) then
        ...
        local header = dwGetglobal(frame:GetName() .. "Header");
...
function SpellTimerFrame_OnUpdate(self, elapse)
    if (self.casting) then
        ...
        local textob = dwGetglobal(self:GetName() .. "Text");
        ...
        if (SpellTimer_Config.ShowProgressBar) then
            local bar = dwGetglobal(self:GetName() .. "Bar");
            local spark = dwGetglobal(bar:GetName() .. "Spark");
            local flash = dwGetglobal(bar:GetName() .. "Flash");
...
function SpellTimer_NewUnitFrame(guid, name)
    local frame;
    for i = 1, SPELLTIMER_MAX_UNITFRAME_COUNT, 1 do
        frame = dwGetglobal("SpellTimerUnitFrame" .. i);
...
function SpellTimer_NewSpellFrame(unit_frame)
    local frame;
    for i = 1, SPELLTIMER_MAX_SPELLFRAME_COUNT, 1 do
        frame = dwGetglobal("SpellTimerSpellFrame" .. i);
        if (frame and not frame:IsVisible()) then
...
do
    ...
    function SpellTimer:AddTimer(guid, target, buff, rank, startTime, duration, delay, texture, count, flags, context, slot, noSync)
        ...
        for i = 1, SPELLTIMER_MAX_SPELLFRAME_COUNT, 1 do
            local frame = dwGetglobal("SpellTimerSpellFrame" .. i);
            local button = dwGetglobal("SpellTimerSpellFrame" .. i .. "Icon");
            ...
            if (frame and frame:IsVisible() and
                ...
                frame.buff = buff;
                frame.count = count;
                local textureOb = dwGetglobal("SpellTimerSpellFrame" .. i .. "IconTexture");
                local countText = dwGetglobal(button:GetName() .. "Count");
        ...
        for i = 1, SPELLTIMER_MAX_UNITFRAME_COUNT, 1 do
            unit_frame = dwGetglobal("SpellTimerUnitFrame" .. i);
            if (unit_frame) then
        ...
        local button = dwGetglobal(spell_frame:GetName() .. "Icon")
        ...
        local textureOb = dwGetglobal(spell_frame:GetName() .. "IconTexture");
        local countText = dwGetglobal(button:GetName() .. "Count");
        textureOb:SetTexture(texture);
        if (count and count > 0) then
            countText:SetText(count);
...
function SpellTimer_RemoveTimer(guid, level, buff, slot)
    ...
    for i = 1, SPELLTIMER_MAX_SPELLFRAME_COUNT, 1 do
        local bar = dwGetglobal("SpellTimerSpellFrame" .. i);
...
function SpellTimer:StartTimer(frame, text, startTime, duration, noSync)
    local bar = dwGetglobal(frame:GetName() .. "Bar");
    local icon = dwGetglobal(frame:GetName() .. "Icon");
    local textob = dwGetglobal(frame:GetName() .. "Text");
            ...
            if (key == spell) then
                ...
                if (pattern[3]) then
                    ...
                    elseif (type(pattern[1]) == "table") then
                        local func = dwGetglobal(pattern[1][1]);
                        time = func(desc, pattern[1][2]);
                        if (time) then
                            time["detect"] = dwGetglobal(pattern[1][3]);
                            if (pattern[1][4]) then -- 切割
                                time["recalc"] = dwGetglobal(pattern[1][4]);
                            end
                            s = 1;
                            e = 1;
                        end
...
local function MobElement_Enable(frame, bool)
    if (bool) then
        frame:Enable();
        dwGetglobal(frame:GetName() .. "Text"):SetTextColor(1.0, 0.82, 0);
        dwGetglobal(frame:GetName() .. "HitArea"):Enable();
    else
        frame:Disable();
        dwGetglobal(frame:GetName() .. "Text"):SetTextColor(0.5, 0.5, 0.5);
        dwGetglobal(frame:GetName() .. "HitArea"):Disable();
    end
end
...
function SpellTimer_EnableOptionEnable(enabled)
    if (enabled) then
        ...
        for i = 1, 8, 1 do
            local frame = dwGetglobal("SpellTimerOptionFrameSpellOption" .. i);
...
function SpellTimer_EnableOption(frame, enabled)
    local checkbox = dwGetglobal(frame:GetName() .. "Checkbox");
    local slider = dwGetglobal(frame:GetName() .. "Slider");
...
function SpellTimerOptionFrameCheckbox_OnClick(self, spellIndex)
    ...
    if (self:GetChecked() == 1) then
        ...
        SpellTimerOptionFrame_EnableSlider(dwGetglobal(self:GetParent():GetName() .. "Slider"));
    else
        if (not SpellTimer_Config.SPELLS) then
            SpellTimer_Config.SPELLS = {};
        end
        ...
        SpellTimerOptionFrame_DisableSlider(dwGetglobal(self:GetParent():GetName() .. "Slider"));
    end
end
...
function SpellTimer_UpdateOption(frame, spell, enabled, delay)
    if (spell) then
        local checkbox = dwGetglobal(frame:GetName() .. "Checkbox");
        local slider = dwGetglobal(frame:GetName() .. "Slider");
        local text = dwGetglobal(checkbox:GetName() .. "Text");
        ...
        if (delay) then
            local sliderText = dwGetglobal(slider:GetName() .. "Text");
...
function SpellTimerList_Update()
    ...
    for i = 1, 8, 1 do
        ...
        local spellFrame = dwGetglobal("SpellTimerOptionFrameSpellOption" .. i);
...
function SpellTimerOptionShowProgress_OnShow(self)
    dwGetglobal(self:GetName() .. "Text"):SetText(SPELL_TIMER_SHOW_PROGRESS);
...
function SpellTimerOptionShowName_OnShow(self)
    dwGetglobal(self:GetName() .. "Text"):SetText(SPELL_TIMER_SHOW_NAME);
...
function SpellTimerOptionFrameSlider_OnChange(self, spellIndex)
    ...
    dwGetglobal(self:GetName() .. "Text"):SetText(delay_time);
...
function SpellTimerOptionFrame_DisableSlider(slider)
    ...
    dwGetglobal(name .. "Thumb"):Hide();
    dwGetglobal(name .. "Text"):SetVertexColor(GRAY_FONT_COLOR.r, GRAY_FONT_COLOR.g, GRAY_FONT_COLOR.b);
...
function SpellTimerOptionFrame_EnableSlider(slider)
    local name = slider:GetName();
    dwGetglobal(name .. "Thumb"):Show();
    dwGetglobal(name .. "Text"):SetVertexColor(NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
