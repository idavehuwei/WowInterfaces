...
if IsConfigurableAddOn("BFTargetFrame") or IsConfigurableAddOn("BuffMaster") or IsConfigurableAddOn("BFCooldown") then
...
if (IsConfigurableAddOn("BFTargetFrame")) then
	ModManagement_RegisterCheckBox(
		...
		function (arg)
			if (arg == 1) then
				if not  BigFoot_IsAddOnLoaded("BFTargetFrame") then
					BigFoot_LoadAddOn("BFTargetFrame")
				end	
				if (BigFoot_IsAddOnLoaded("BFTargetFrame")) then
					LibStub("AceAddon-3.0"):GetAddon("BFTargetFrame"):Enable()
				end
			else
				if (BigFoot_IsAddOnLoaded("BFTargetFrame")) then
					LibStub("AceAddon-3.0"):GetAddon("BFTargetFrame"):Disable()
				end
			end
		end
	)
