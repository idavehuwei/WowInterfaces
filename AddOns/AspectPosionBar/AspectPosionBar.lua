...
function A:AspectBarUpdate()
    ...
    if (numForms > 0) then
        if ( numForms == 1 ) then
            ...
            dwSecureCall(AspectPosionBarButton1.ClearAllPoints, AspectPosionBarButton1);
            dwSecureCall(AspectPosionBarButton1.SetPoint, AspectPosionBarButton1, "BOTTOMLEFT", "AspectPosionBar", "BOTTOMLEFT", 12, 3);
        ...
        dwSecureCall(AspectPosionBar.Show, AspectPosionBar);
    else
...
function A:AspectBarUpdateState()
    ...
    for i=1, NUM_SHAPESHIFT_SLOTS do
        ...
        if ( i <= numForms ) then
            ...
            if (type == "spell") then
                dwSecureCall(button.SetAttribute, button, "type", "spell");
                dwSecureCall(button.SetAttribute, button, "spell", name);
                button.spellID = self.actions.spells[i];
            elseif (type == "teleport") then
                dwSecureCall(button.SetAttribute, button, "type", "spell");
                dwSecureCall(button.SetAttribute, button, "spell1", L["传送: "] .. name);
                dwSecureCall(button.SetAttribute, button, "spell2", L["传送门: "] .. name);
                button.teleportID = true;
            elseif (type == "item") then
                dwSecureCall(button.SetAttribute, button, "type", "item");
                dwSecureCall(button.SetAttribute, button, "item", name);
                dwSecureCall(button.SetAttribute, button, "target-slot1", "16");
                dwSecureCall(button.SetAttribute, button, "target-slot2", "17");
...
function A:UIParent_ManageFramePositions()
    ...
    dwSecureCall(AspectPosionBar.ClearAllPoints, AspectPosionBar);
    dwSecureCall(AspectPosionBar.SetPoint, AspectPosionBar, "BOTTOMLEFT", "MainMenuBar", "TOPLEFT", 30, YOffset);
end
-----------------------------------------
--  OnX
-----------------------------------------
function A:OnModuleEnable()
    ...
    dwSecureCall(ShapeshiftBarFrame.Hide, ShapeshiftBarFrame);
...
function AspectPosionBar_Jieguan(switch)
    ...
    for i=1, NUM_SHAPESHIFT_SLOTS do
        ...
        dwSecureCall(apb.ClearAllPoints, apb);
        if (switch) then
            dwSecureCall(apb.SetPoint, apb, "CENTER", ssb, "CENTER", 0, 0);
        else
            if (i == 1) then
                dwSecureCall(apb.SetPoint, apb, "BOTTOMLEFT", AspectPosionBar, "BOTTOMLEFT", 10, 3);
            else
                dwSecureCall(apb.SetPoint, apb, "LEFT", la, "RIGHT", 8, 0);
            end
            la = apb;
        end
