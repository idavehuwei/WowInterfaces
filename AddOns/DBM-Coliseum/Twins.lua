...
function mod:OnCombatStart(delay)
    ...
    sndWOP:Schedule(40-delay, "Interface\\AddOns\\DBM-Core\\extrasounds\\twinsoon.mp3")
...
function mod:SPELL_CAST_START(args)
    if args:IsSpellID(66046, 67206, 67207, 67208) then      -- Light Vortex
        ...
        if not debuff then
            sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\changewhite.mp3")
        end
    ...
    elseif args:IsSpellID(66058, 67182, 67183, 67184) then  -- Dark Vortex
        ...
        if not debuff then
            sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\changeblack.mp3")
        end
        self:SpecialAbility(debuff)
    elseif args:IsSpellID(65875, 67303, 67304, 67305) then  -- Twin's Pact
        sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\blackshield.mp3")
        timerHeal:Start()
    ...
    elseif args:IsSpellID(65876, 67306, 67307, 67308) then  -- Light Pact
        sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\whiteshield.mp3")
        timerHeal:Start()
        self:SpecialAbility(true)
...
function mod:SpecialAbility(debuff)
    ...
    sndWOP:Schedule(40, "Interface\\AddOns\\DBM-Core\\extrasounds\\twinsoon.mp3")
end
