...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(15-delay, "Interface\\AddOns\\DBM-Core\\extrasounds\\doorsoon.mp3")
	warnPortalSoon:Schedule(15-delay)
	timerVolcanoCD:Start(80-delay)
	sndWOP:Schedule(75-delay, "Interface\\AddOns\\DBM-Core\\extrasounds\\volcanosoon.mp3")
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(67051, 67050, 67049, 66237) then			-- Incinerate Flesh
		...
		if mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\healflesh.mp3")
		end
		if self.Options.IncinerateFleshIcon then
	...
	elseif args:IsSpellID(66334, 67905, 67906, 67907) and args:IsPlayer() then
		specWarnKiss:Show()
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\breakcast.mp3")
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(67901, 67902, 67903, 66258) then		-- Infernal Volcano
		if mod:IsDifficulty("heroic10") or mod:IsDifficulty("heroic25") then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\killvolcano.mp3")
		end
		timerVolcanoCD:Start()
		sndWOP:Schedule(115, "Interface\\AddOns\\DBM-Core\\extrasounds\\volcanosoon.mp3")
		warnVolcanoSoon:Schedule(110)

	elseif args:IsSpellID(67900, 67899, 67898, 66269) then		-- Nether Portal
		if mod:IsDifficulty("heroic10") or mod:IsDifficulty("heroic25") then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\killdoor.mp3")
		end
		timerPortalCD:Start()
		sndWOP:Schedule(115, "Interface\\AddOns\\DBM-Core\\extrasounds\\doorsoon.mp3")
