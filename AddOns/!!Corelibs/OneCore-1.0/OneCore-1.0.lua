...
local MAJOR, MINOR = "OneCore-1.0", tonumber("40") or 9999
...
function OneCore:ParseItemLink(link)
    if (link) then
...
function OneCore:IsQuestItem(link, bag, slot)
    if (link) then
        local id = OneCore:ParseItemLink(link);
        if (id) then
            if (OneCoreIDQuest and OneCoreIDQuest[id]) then
                if (OneCoreIDQuest[id] == 1) then
                    return true;
                else
                    return false;
                end
            ...
            OneCoreTooltip:SetBagItem(bag, slot);
            -- GameTooltip:SetBagItem(bag, slot);

            if (not OneCoreIDQuest) then
                OneCoreIDQuest = {};
            end

            local textLeft2 = OneCoreTooltipTextLeft2:GetText();
            -- local textLeft2 = GameTooltipTextLeft2:GetText();
            -- print("textLeft2 = "..tostring(textLeft2));
            if (textLeft2 == ITEM_BIND_QUEST) then
                OneCoreIDQuest[id] = 1;
                return true;
            else
                OneCoreIDQuest[id] = 0;
...
function OneCore:ColorSlotBorder(slot, fcolor)
    ...
    if self.db.profile.appearance.rarity and not fcolor and not bcolor then
        ...
        if link then
            ...
            if(self.db.profile.appearance.questitem) then
                if(OneCore:IsQuestItem(link, bag, slot)) then
                    color = colorCache[questitem]
                    if not color then
                        color = {r = 0.0, g = 0.8, b = 0.8}
                        colorCache[questitem] = color
                    end
                end
            end
        end
