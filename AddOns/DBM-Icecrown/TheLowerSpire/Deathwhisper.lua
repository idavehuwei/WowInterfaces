...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(2, "Interface\\AddOns\\DBM-Core\\extrasounds\\mobsoon.mp3")
...
function mod:addsTimer()
	timerAdds:Cancel()
	warnAddsSoon:Cancel()
	sndWOP:Cancel("Interface\\AddOns\\DBM-Core\\extrasounds\\mobsoon.mp3")
	if mod:IsDifficulty("heroic10") or mod:IsDifficulty("heroic25") then
		warnAddsSoon:Schedule(40)	-- 5 secs prewarning
		self:ScheduleMethod(45, "addsTimer")
		timerAdds:Start(45)
		sndWOP:Schedule(40, "Interface\\AddOns\\DBM-Core\\extrasounds\\mobsoon.mp3")
	else
		warnAddsSoon:Schedule(55)	-- 5 secs prewarning
		self:ScheduleMethod(60, "addsTimer")
		timerAdds:Start()
		sndWOP:Schedule(55, "Interface\\AddOns\\DBM-Core\\extrasounds\\mobsoon.mp3")
	end
end
...
function mod:SPELL_CAST_START(args)
	...
	elseif args:IsSpellID(70900) then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\transmob.mp3")
	...
	elseif args:IsSpellID(70901) then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\powermob.mp3")
...
function mod:SPELL_SUMMON(args)
	if args:IsSpellID(71426) then -- Summon Vengeful Shade
		if time() - lastSpirit > 5 then
			...
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\spirits.mp3")
			lastSpirit = time()
		end
	end
end
