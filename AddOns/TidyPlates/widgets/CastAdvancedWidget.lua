...
--[[
The Widget:
    - Looks at a table, or uses a function to determine if the plate refers to a unit that is associated with a spell cast.
    - Each Widget registers itself with the Watcher frame; The watcher frame will trigger a warning, if possible.
    - Each Widget will cache the name, and (if possible) the GUID of the unit
    - The widget warning handler will compare the widget's ID with the warning ID
    - if the unit gets a unitid (mouseover or target) during a warning, the cast bar is shown
    - Upon Mouseover
--]]
