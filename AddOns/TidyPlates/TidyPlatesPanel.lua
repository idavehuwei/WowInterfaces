local L = LibStub("AceLocale-3.0"):GetLocale("TidyPlates")
    ...
    if type(theme) == 'table' then
        if theme.SetStyle and type(theme.SetStyle) == "function" then
            ...
            for propertyname, oldvalue in pairs(TidyPlates.defaultTheme) do
                ...
                else theme[propertyname] = copytable(oldvalue)
                end
            end
        end
...
local panel = PanelHelpers:CreatePanelFrame("TidyPlatesInterfaceOptions", L["Tidy Plates"], title)
local helppanel = PanelHelpers:CreatePanelFrame("TidyPlatesInterfaceOptionsHelp", L["Troubleshooting"])
...
local function ActivateInterfacePanel()
    --- - Note
    local offset = -20
    panel.label = panel:CreateFontString(nil, 'ARTWORK', 'GameFontNormal') --, 'GameFontLarge'
--    panel.label:SetFont("Fonts\\ZYKai_T.TTF", 13, nil)
    panel.label:SetPoint("TOPLEFT", panel, "TOPLEFT", 35, -40)
    panel.label:SetWidth(340)
    panel.label:SetJustifyH("LEFT")
    panel.label:SetText(L["Please choose a theme for your Primary and Secondary Specializations; The appropriate theme will be automatically activated when you switch specs."])
    panel.label:SetTextColor(1, 1, 1, 1)

    -- Theme Choices
    for index, name in pairs(TidyPlatesThemeNames) do
        dropmenu[index] = { text = name, notCheckable = 1 }
    end

    -- Primary Spec Dropdown
    panel.PrimarySpecTheme = PanelHelpers:CreateDropdownFrame("TidyPlatesChooserDropdown", panel, dropmenu, 1)
    panel.PrimarySpecTheme:SetPoint("TOPLEFT", panel.label, "BOTTOMLEFT", -20, offset - 10)
    ...
    -- Overlap:
    panel.AllowOverlap = PanelHelpers:CreateCheckButton("TidyPlatesOptions_Overlap", panel, L["Allow Overlapping Nameplates"])
    ...
    panel.AllowOverlap:SetScript("OnClick", function(self)
    ...
    -- [[ Autohide:
    panel.AutoHide = PanelHelpers:CreateCheckButton("TidyPlatesOptions_Autohide", panel, L["Show/Hide Plates when Combat Starts and Ends"])
    panel.AutoHide:SetPoint("TOPLEFT", panel.AllowOverlap, "TOPLEFT", 0, -35)
    --]]

    --[[
    -- Autoshow:
    panel.AutoShow = PanelHelpers:CreateCheckButton("TidyPlatesOptions_AutoShow", panel, L["Show Enemy Nameplates when Combat Starts"])
    panel.AutoShow:SetPoint("TOPLEFT", panel.AllowOverlap, "TOPLEFT", 0, -35)

    -- Autohide:
    panel.AutoHide = PanelHelpers:CreateCheckButton("TidyPlatesOptions_AutoHide", panel, L["Hide Enemy Nameplates when Combat Ends"])
    ...
    panel.refresh = function()
        ...
        if tonumber(GetCVar("nameplateAllowOverlap")) == 1 then
            panel.AllowOverlap:SetChecked(true)
        else
            panel.AllowOverlap:SetChecked(false)
        end
...
helppanel.parent = L["Tidy Plates"]
...
local function ActivateHelpPanel()
    ...
    --helppanel.vkeylabel:SetFont("Fonts\\ZYKai_T.TTF", 13, nil)
    ...
    helppanel.vkeylabel:SetText(L["Show Nameplates:|cFFFFFFFF You can also toggle nameplates on-and-off by using: '|rV|cFFFFFFFF' for Enemies, '|rShift-V|cFFFFFFFF' for friends, and '|rControl-V|cFFFFFFFF' for both."])
    ...
    --helppanel.resetreloadlabel:SetFont("Fonts\\ZYKai_T.TTF", 13, nil)
    ...
    helppanel.resetreloadlabel:SetText(L["Reset Variables:|cFFFFFFFF This button will reset Tidy Plates variables, and reload the UI. Note: Using this button during combat is NOT a good idea. "])
    ...
    helppanel.resetreload:SetText(L["Reset"])
    ...
    --helppanel.notes:SetFont("Fonts\\ZYKai_T.TTF", 13, nil)
    ...
    helppanel.notes:SetText(L["If issues persist, visit our page at Wowinterface.com to file a bug report or to ask a question. If you're filing a bug, please use the 'Report Bug' button."])
...
ApplyPanelSettings = function()
    ...
    else SetCVar("nameplateAllowOverlap", 0)
    end
end
...
TidyPlatesSlashCommands.reset = function()
    print(L["Tidy Plates: Variables have been reset"]);
    TidyPlatesOptions = copytable(TidyPlatesOptionsDefaults);
    LoadTheme(TidyPlatesOptions[activespec])
end
...
function slash_TidyPlates(arg)
    if type(TidyPlatesSlashCommands[arg]) == 'function' then
        ...
        TidyPlates:ForceUpdate()
    else InterfaceOptionsFrame_OpenToCategory(panel)
    end
end
